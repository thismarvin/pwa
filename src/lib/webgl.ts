...
export function createProgram(
	...
	const fragmentShader = _createShader(gl, ShaderType.Fragment, fragmentShaderSource);
...
export function createBufferInit(
	...
	switch (usage) {
		case BufferUsage.Vertex: {
			const buffer = allocateVertexBuffer(gl, contents.byteLength, VertexUsage.Static);
		...
		case BufferUsage.Index: {
			const buffer = allocateIndexBuffer(gl, contents.byteLength, VertexUsage.Static);
...
function _createShader(gl: RenderingContext, type: ShaderType, source: string): WebGLShader {
	...
	if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
		const error = new Error(
			`An error occurred will compiling the shader: ${gl.getShaderInfoLog(shader)}`
		);
...
function _createProgram(
	...
	if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
		throw new Error(
			`An error occurred while initializing the shader program: ${gl.getProgramInfoLog(program)}`
		);
	}
