...
function _createOrthographicCamera(
	...
	const view = Matrix4.createLookAt(position, Vector3.add(position, forward), up);
...
export default class Pixels {
	...
	constructor(canvas: HTMLCanvasElement, width: number, height: number) {
		...
		const vertexPositions = new Int8Array([0, 0, 0, 0, -1, 0, 1, -1, 0, 1, 0, 0]);
		...
		);
		{
			...
			this.gl.vertexAttribPointer(index, 3, WebGL.AttributeType.Byte, false, 3, 0);
		...
		);
		{
			...
			this.gl.bindBuffer(WebGL.BufferUsage.Vertex, this.vertexTranslationsBuffer);
			this.gl.enableVertexAttribArray(index);
			this.gl.vertexAttribPointer(index, 3, WebGL.AttributeType.Short, false, 6, 0);
		...
		);
		{
			...
			this.gl.enableVertexAttribArray(index);
			this.gl.vertexAttribPointer(index, 4, WebGL.AttributeType.UnsignedByte, true, 4, 0);
			this.gl.vertexAttribDivisor(index, 1);
		}

		this.indexBuffer = WebGL.createBufferInit(this.gl, WebGL.BufferUsage.Index, indices);
		{
	...
	public draw(): void {
		if (this.dirty) {
			WebGL.setVertexBufferData(this.gl, this.vertexColorsBuffer, this.vertexColors);

			this.dirty = false;
		}

		this.gl.drawElementsInstanced(WebGL.DrawMode.Triangles, 6, this.gl.UNSIGNED_BYTE, 0, this.area);
	}
}
